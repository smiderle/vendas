...
vendasApp.controller('OrderListController',
		['$scope', '$location', '$compile' ,'OrderService','ContextService','DateUtil',
		 function OrderListController($scope,$location, $compile, OrderService, ContextService, DateUtil) {
			...
			$scope.listOrders = function(){
				...
				if($scope.filter){
					cOrders = OrderService.getByFilter( $scope.filter, organizationID, branchID, $scope.orders.length);					
				} else {
					cOrders = OrderService.getAllByBranch(organizationID,branchID, $scope.orders.length);

...
vendasApp.controller('PaymentListController',['$scope','$location','PaymentService' ,'ContextService','UtilityService','FormsPaymentService', 'DateUtil',
	...
	$scope.listPedingPayments = function(){
		...
		if($scope.complexFilter){
			...
			cPayments = PaymentService.getByComplexFilter( organizationID, branchID, cf.customerID, DateUtil.getTime(cf.issuanceDateGte), DateUtil.getTime(cf.issuanceDateLte), DateUtil.getTime(cf.expirationDateGte), DateUtil.getTime(cf.expirationDateLte), cf.status, cf.formPayment, $scope.payments.length);
		} else if($scope.filter){
			cPayments = PaymentService.getPendingByFilter( organizationID, branchID, $scope.filter, $scope.payments.length);					
		} else {
			cPayments = PaymentService.getAllPendingByOrganizationID(organizationID,branchID, $scope.payments.length);
